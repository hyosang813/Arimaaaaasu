...
//ボタン生成
- (void)makeButton:(NSString *)text sel:(SEL)sel rect:(CGRect)rect tag:(int)tag
{
...
- (void)viewDidLoad
{
    ...
    //選択モード識別変数を念のため101「EASY」で初期化しとこう
    mode = EASY;
    
    //メインの状態フラグを0「START」(シャーレを左外から中央に持って来る状態)で初期化しとこう
    mainFlg = BORN;
    ...
    //twitter連携ボタン配置
    twitterButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.bounds.size.width / 2 - 80, 330, 40, 40)];
    ...
    //facebook連携ボタン配置
    facebookButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.bounds.size.width / 2 + 40, 330, 40, 40)];
...
- (void)modeInit
{
    switch (modeStatus) {
        case START: //スタート画面
            ...
            SEL pressButton = NSSelectorFromString(@"pressButton:");
            [self makeButton:@"EASY    MODE" sel:pressButton rect:CGRectMake(self.view.bounds.size.width / 2 - 100, 220, 200, 40) tag:EASY];
            [self makeButton:@"NOMAL MODE" sel:pressButton rect:CGRectMake(self.view.bounds.size.width / 2 - 100, 260, 200, 40) tag:NOMAL];
            [self makeButton:@"HARD    MODE" sel:pressButton rect:CGRectMake(self.view.bounds.size.width / 2 - 100, 300, 200, 40) tag:HARD];
        ...
        case PLAY: //ゲームプレイ画面
        {   //「イージーモード」、「ノーマルモード」、「ハードモード」の各ボタンを削除 ※Switch分の中で変数宣言するためにはブロック({})で囲わないとエラーになるんだって　へーへーへー
            NSMutableArray *array = [NSMutableArray array];
        ...
        {   //小保方さんの「土台」を生成　本体画像などはOboLayerクラス側に記述
            ...
            //心臓タイマー
            heartBeatTimer = [NSTimer scheduledTimerWithTimeInterval:.01 target:self selector:@selector(mainBatch) userInfo:nil repeats:YES];
        ...
        {   //クリアタイムラベル、クリアモードラベル、リスタートボタンの生成
            ...
            SEL pressButton = NSSelectorFromString(@"pressButton:");
            [self makeButton:@"RETRY" sel:pressButton rect:CGRectMake(self.view.bounds.size.width / 2 - 100, 310, 200, 40) tag:RETRY];
        }
            //ソーシャルボタン表示
...
- (void)pressButton:(UIButton *)sendar
{
    if (sendar.tag == RETRY) {
        ...
        mode = (int)sendar.tag; //イージーとノーマルのどっちが選択されたかをmode変数に格納しといて後でプログレスバー生成有無に使う
...
- (void)displayElapsed
{
    ...
    timeLabel.text  = [NSString stringWithFormat:@"TIME : %02d:%02d:%02d", minite, second, miliSecond];
}
...
- (void)mainBatch
{
    switch (mainFlg) {
        ...
        case DIE: //長押しが完了したタイミング状態
            ...
            //長押し秒数を初期化　イージーモードはプログレスバー表示も０に初期化
            summary = 0;
            if (mode == EASY) {
...
- (void)makeAnimation:(CGRect)rect
{
    ...
    // 終了状態 配置場所(中央)まで移動
    anime.toValue = [NSNumber numberWithInt:0];
    
    //細胞が存在しなければ(START時点だったら)し、存在すれば(END時点だったら)再利用
    CellLayer *cell = (CellLayer *)[self findLayer:@"CELL"];
    
    //存在しなかったら細胞インスタンスを生成し、存在すれば再利用して中央から右へ消す
    if (cell == nil) {
        
        // アニメーション設定　開始状態 X座標を-185(320 / 2 + 25)から〜
        anime.fromValue = [NSNumber numberWithInt:-185 - 100];
        ...
        anime.fromValue = [NSNumber numberWithInt:0];
...
//(delegate設定されている)アニメーション終了時にコールされる ※細胞生成アニメーションだったらモードをPLAYに、細胞死亡アニメーションだったらモードをSTARTへ
